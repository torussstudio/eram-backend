...
export const getLatestEventTimestamp = async (req, res) => {
  try {
    const latest = await Event.findOne({ institution: "sitewide" })
      .sort({ createdAt: -1 })
      .select("createdAt");
    res.json({ latestTimestamp: latest ? latest.createdAt : null });
  } catch (err) {
    res.status(500).json({ message: "Failed to fetch latest timestamp" });
  }
};